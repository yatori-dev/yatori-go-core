...
func CourseDetailAction(cache *yinghuaApi.YingHuaUserCache, courseId string) (YingHuaCourse, error) {
	courseDetailJson, err := cache.CourseDetailApi(courseId, 30, nil)
	if err != nil {
		return YingHuaCourse{}, errors.New("获取数据失败:" + err.Error())
	}
...
func VideosListAction(UserCache *yinghuaApi.YingHuaUserCache, course YingHuaCourse) ([]YingHuaNode, error) {
	...
	//接口一爬取视屏信息
	listJson, err := yinghuaApi.CourseVideListApi(*UserCache, course.Id, 30, nil)
	if err != nil {
	...
	//如果获取失败
	if gojsonq.New().JSONString(listJson).Find("msg") != "获取数据成功" {
		return []YingHuaNode{}, errors.New("获取数据失败：" + listJson)
	}
	jsonList := gojsonq.New().JSONString(listJson).Find("result.list")
	// 断言为切片并遍历
	if items, ok := jsonList.([]interface{}); ok {
...
func aiTurnYingHuaAnswer(cache *yinghuaApi.YingHuaUserCache, aiAnswer string, v entity.YingHuaExamTopic) []string {
	...
	if v.Type == qtype.SingleChoice.String() || v.Type == qtype.TrueOrFalse.String() || v.Type == qtype.MultipleChoice.String() {
		...
		for _, item := range jsonStr {
			...
			result := qutils.SimilarityArraySelect(item, v.Question.Options)
			res = append(res, result)
		}

		answer = res
	} else if v.Type == qtype.FillInTheBlank.String() || v.Type == qtype.ShortAnswer.String() {
		var res []string
